...
from .hls_utils import fetch_binary, fetch_text, parse_media_playlist, probe_segment_timeline
...
def test_copied_audio_deep_seek_is_not_slow(master_context: dict, test_config) -> None:
    ...
    for audio in master_context["audios"]:
        ...
        tested += 1
        target_idx = min(int(len(playlist.segment_urls) * 0.85), len(playlist.segment_urls) - 2)
